package com.xpayback.usersquicklist.utils
...
class InternetConnectivityUtils(val context: Context) : LiveData<Boolean>() {
    ...
    fun updateConnection() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ...
            capabilities?.run {
                Connnectserver =
                    capabilities.hasTransport(TRANSPORT_CELLULAR) || capabilities.hasTransport(
                        TRANSPORT_WIFI
                    )
            }
        } else {
